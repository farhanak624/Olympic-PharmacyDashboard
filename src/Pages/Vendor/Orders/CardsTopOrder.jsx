...
function Card({ text, icon, data, color, bgcolor }) {
  ...
  return (
    <div className="rounded-lg shadow-md p-3 bg-subContainerColor">
      ...
      <div className="mt-3 mb-3">
        <p className="text-textColor text-xs">{text}</p>
      </div>
      <div className="w-full font-bold flex gap-1  text-xl items-center">
        {data ? (
          <p className="text-2xl text-textColor">{data}</p>
        ) : (
          <p className="text-2xl text-textColor">0</p>
        )}
      </div>
    </div>
  );
